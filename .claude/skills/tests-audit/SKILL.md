...
4. **Run the CI gate locally — before opening the PR.** The PR auto-merges on a green run (step 7), so it must already be green. `main`'s required checks are `test`, `typecheck`, and `lint-and-format`. Run, in order:
   1. `pnpm prettier:format` — auto-fix formatting of the files you just created/edited.
   2. `pnpm check` — `typecheck` + `lint` + `prettier` (check mode) + `knip`.
   3. `pnpm test -- --run` — the full suite, including any test you just added.
...
5. **Commit.** One commit, message `test: audit and refresh test suite (YYYY-MM-DD)`. List affected files and a one-line summary of each change in the body. Include formatting-only changes from `pnpm prettier:format` in the same commit.
