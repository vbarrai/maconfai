...
it('returns the SHA of a matching folder', async () => {
  const { fetchSkillFolderHash } = await import('../../../src/lock.ts')

  mockFetch.mockResolvedValueOnce(
    okTree([{ path: 'skills/my-skill', type: 'tree', sha: 'folder-sha-123' }]),
  )
  ...
  expect(result).toMatchInlineSnapshot(`"folder-sha-123"`)
})
