...
it('normalizes backslashes in skill path to forward slashes', async () => {
  const { fetchSkillFolderHash } = await import('../../../src/lock.ts')

  mockFetch.mockResolvedValueOnce(
    okTree([{ path: 'skills/win-skill', type: 'tree', sha: 'win-sha' }]),
  )

  const result = await fetchSkillFolderHash(
    'owner/repo',
    'skills\\win-skill\\SKILL.md',
    'token',
    'main',
  )

  expect(result).toMatchInlineSnapshot(`"win-sha"`)
})
