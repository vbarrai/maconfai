...
it('should create a URL MCP server in dedicated directory', async () => {
  promptResponses = [
    'mcp', // select: config type
    'linear', // text: name
    'url', // select: transport
    'https://mcp.linear.app/sse', // text: url
    false, // confirm: headers
  ...
  const content = await readFile(join(tempDir, 'mcps/linear/mcp.json'), 'utf-8')
