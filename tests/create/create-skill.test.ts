...
it('should create a skill with SKILL.md', async () => {
  promptResponses = [
    'skill', // select: config type
    'my-skill', // text: name
    'A useful skill', // text: description
    'Do something cool', // text: content
  ]

  const { runCreate } = await import('../../src/create.ts')
  await runCreate()

  const content = await readFile(join(tempDir, 'skills/my-skill/SKILL.md'), 'utf-8')
